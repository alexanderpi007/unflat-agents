...
export function agentServer(call: (name: AgentTool, input: Record<string, unknown>, requestInfo?: RequestInfo) => Promise<unknown>, moneyMode?: string) {
  ...
  const money = " Requires a valid mandate. After expiry the call returns REFUSED with a reason: this is the expected outcome, not an error—do not retry. Base uses real mainnet USDC; avalanche-fuji uses valueless testnet USDC and AVAX gas. money_mode=mock never broadcasts. Never resubmit a payment with the same idempotency_key or change keys to retry an uncertain payment.";
  ...
  register("get_account", "Opens a new account without Authorization when no account is bound/authenticated. Ask the owner for name and owner_email if missing; never invent their email. The response includes account_token, shown ONCE; save it privately. Also returns funding_address to give the owner: fund Base accounts with Base USDC and ETH gas; fund avalanche-fuji accounts with Fuji test USDC and AVAX gas. Successful get_account binds this MCP session (session_bound=true); later calls need no repeated token or URL change. To reconnect, pass your saved account_token and omit name/owner_email: this reads and binds the existing account, without creating another wallet. Read chain, balance, money_mode and mandate.allowed. Do not fund mock addresses. Five enrollments/IP/hour, ten total/hour; no automatic provisioning retry. Existing names, including Atlas, cannot be claimed.", {
    ...
    chain: z.enum(["base", "avalanche-fuji"]).optional().describe("Enrollment chain; default base. avalanche-fuji supports pay only, with Circle test USDC and AVAX gas. Cannot change an existing account's chain. Its Privy EVM wallet address is unchanged across networks."),
  });
  ...
  register("strategize", "Base only: run dry then fee-waived AIMorgan advice under mandate. Advice is advisory only. Fuji supports pay only; strategize returns REFUSED: not supported on this chain. Requires account authentication or a bound session, plus a valid mandate; expiry means stop, not retry.", keys);
  register("save", "Base only: save exactly 100 USDC cents ($1.00) to the owner-allowlisted vault, with dry/free advice, exact approval, simulation and deposit. On avalanche-fuji returns REFUSED: not supported on this chain; no advice, approval or deposit is attempted." + money, { amountUsdcCents: z.literal(100).describe("100 cents = $1.00 USDC"), ...keys });

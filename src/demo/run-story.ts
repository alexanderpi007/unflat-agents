...
export function runStory({ agent, mandate, events }: DemoSnapshot) {
  ...
  if (done("mandate.grant")) lines.push({ speaker: "Owner", text: `You have approximately two minutes and a ${amount(mandate.maxTotalUsdcCents)} budget.` });
  ...
  if (events.some(e => e.action === "usdc.transfer" && e.status === "refused" && /Arkiv returned no matching unexpired entity/.test(e.reason))) {
    lines.push({ speaker: "Agent", text: `Time ran out. My next payment was refused, with ${amount(mandate.maxTotalUsdcCents - mandate.spentUsdcCents)} left in the budget.` });
  }
  return lines;
}

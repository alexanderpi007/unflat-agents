...
const inputSchema = z.object({ displayName: z.string().min(1).max(36), owner_email: z.string().trim().toLowerCase().email().max(254), chain: z.enum(["base", "avalanche-fuji"]).optional() }).strict();
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await enroll(runtime, parsed.data.displayName, parsed.data.owner_email, undefined, parsed.data.chain), { status: 201 });
  } catch (error) {

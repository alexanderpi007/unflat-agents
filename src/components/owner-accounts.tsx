...
type Row = { id: string; name: string; fundingAddress: string | null; status: string;
...
export function OwnerAccounts({ token, onGranted }: { token: string; onGranted: () => void }) {
  ...
  return <section aria-label="Owner accounts">
    ...
    {data?.accounts.map(account => <article key={account.id}>
      ...
      <details><summary>Grant details</summary><p>{account.mandate.reason}</p><p>Two minutes, $1.20 total cap, $1.00 per action. Pay 0.05 USDC to {data.recipient}; save 1.00 USDC into {data.vault}. Repeated actions allowed within the cap; plus Base gas when live.</p></details>

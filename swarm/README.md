...
3. Use the selected archived statement, or click **Run a simulation →** at the bottom. The dashboard simulation uses mocked financial actions and a real Arkiv mandate; wait for natural expiry and refusal. The separate Publish button uses real owner storage even if `MOCK_MODE=true`.
...
5. Save the complete **YOUR PRIVATE RETRIEVAL REFERENCE** using **Copy secret reference**. Record the byte count, source label, and estimated drive retention if available. Save the reference outside this browser session.

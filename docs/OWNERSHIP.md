...
2. The agent connects anonymously to the MCP URL and calls `get_account({"name":"nova","owner_email":"owner@example.com"})`.
...
5. The agent receives the funding address and its per-account token **once**. It saves the token privately. Successful get_account binds the MCP session; later tools need no repeated token. In a new session, pass the saved account_token argument to get_account to restore that account without enrollment. Bearer/query credentials still work. The bank stores only the token hash; temporary sessions hold the account ID and fingerprint, never raw tokens. The owner sees the email, ownership label and address in Owner mode.
...
7. The agent requests a mandate. The matching Privy email owner approves at the returned `approval_url` with typed `CONFIRM`: two minutes, $1.20 total cap, $1.00 per action. The Arkiv entity, ENS commitment record and fresh authorization queries work as before.
8. The agent pays or saves through its account token or bound MCP session. The gateway checks Arkiv, ENS, budget, AIMorgan price validation and independent preflight before signing. The agent has neither the wallet key nor the session-signing authorization key.
...
**Gateway Owner mode:** sign in using Privy email OTP. The server verifies the access token, fetches its Privy user, and requires both a verified email matching `owner_email` and the stored wallet-owner user ID. Only that user's accounts and requests are visible or actionable. The request link contains only its ID and opens a single-request view. `OWNER_TOKEN` remains API-only bank-operator authority over all accounts; it is never entered in this UI. Pending approval and grant remain CONFIRM-gated. A direct **Grant budget** for an owner-owned account also permits the owner-only `earn.recall` and `owner.transfer` operations; approving an agent's pending request does not add those operations. Both recovery operations require another typed CONFIRM and a fresh unexpired mandate, ENS resolution, AIMorgan `allPassed === true`, and preflight. Recall redeems a specified raw share quantity into the same account wallet, with no approval transaction and no consumption of the spending cap. Transfer-to-owner sends 1 USDC in the UI (API: 1–100 cents) to the external address explicitly entered by the owner; that address is not cryptographically verified against the email. Grant a fresh budget after expiry. Agent credentials cannot invoke these endpoints or grant permission.

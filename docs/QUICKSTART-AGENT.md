...
For real money, use `MOCK_MODE=false`, the existing complete Privy/Base/AIMorgan/Arkiv configuration, `AIMORGAN_X402=false`, and `EARN_VIA_PRIVY=false`. New accounts additionally require `PRIVY_SESSION_SIGNER_ID`, a key quorum matching `PRIVY_AUTHORIZATION_PRIVATE_KEY`, email login enabled in Privy, and the current tunnel origin in Privy's allowed origins. Follow [ownership setup](OWNERSHIP.md) before enrollment. Fund each new account separately; Atlas's balance is not shared. Owner approvals display the actual money mode. Missing live prerequisites refuse execution; `MOCK_MODE=true` makes MCP money mocked. `demo:mock` stays completely mocked. Do not run another gateway process or CLI transaction writer against the same file store during the demo.
...
3. In **All accounts**, verify the enrolled name, owner email and funding address. Open **Log in on Privy to withdraw or revoke** and verify that the same email login exposes this wallet before funding it. This is separate from OWNER_TOKEN. Fund only a **ready**, live account with Base USDC and ETH for gas from your own wallet. Unknown balances say unavailable, not zero. Either grant that account a budget directly, or wait for its named pending request in **Owner approvals** (refreshes every five seconds).
...
New wallets belong to the specified Privy email user, not to the app or the holder of OWNER_TOKEN. **Owner recovery** in an account card can recall vault shares to that same wallet or transfer 1 USDC to a reviewed address. These gateway actions need a new direct owner grant if the budget expired, and their own CONFIRM. The separate `/owner-wallet` screen authenticates the actual Privy owner and offers direct withdrawal, isolated key export and removal of additional signers. Human-owner control does not depend on an agent mandate. That screen handles real money even if the agent demo is mocked; it requires an explicit wallet selection and CONFIRM. Back up a key privately before relying on recovery if the bank disappears; the bank-hosted page itself is not independent recovery. [Full limits and instructions](OWNERSHIP.md).
...
Connect with that token and call `get_account {"name":"nova","owner_email":"owner@example.com"}`, using the owner's actual email. Names are normalized to lowercase, 1–36 characters, starting with a letter; digits and internal hyphens are allowed. This pregenerates a distinct wallet for that Privy email user, attaches the gateway's limited signer, and creates `nova.agents.unflat.eth`. An existing name (including Atlas) is refused before wallet creation. Email is required for enrollment, not for later scoped reads; it cannot be changed with the account token.
...
The operator manages approvals for every account through `OWNER_TOKEN`; each wallet's actual owner authenticates separately through Privy with their email. A persistent public owner ID goes in the ENS `owner` record—not the email, secret or its hash. ENS also records the wallet address, gateway URL and current mandate commitment (zero hash until the first grant). Parent/deployer and gateway ENS roles remain unchanged.
